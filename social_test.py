...
def init_db():
    ...
    for friend in friends:
        try:
            c.execute("INSERT INTO friends (user_id, friend_id, status) VALUES (?, ?, ?)", friend)
        except:
            pass
    ...
    messages = [
        ...
        (1, 3, 'Ты посмотрел мой новый проект?'),
        (3, 1, 'Да, отличная работа!'),
        (2, 3, 'Где мы встретимся?'),
        (3, 2, 'Предлагаю в кафе на главной.'),
    ]
    
    for message in messages:
        try:
            c.execute("INSERT INTO messages (sender_id, receiver_id, content) VALUES (?, ?, ?)", message)
        except:
            pass
...
# Добавляем все HTML шаблоны как строки
templates = {
...
<body>
    ...
    <div class="content">
        ...
        <div class="chat-container">
            ...
            <form class="chat-input" method="POST" action="{{ url_for('messages', friend_id=friend_id) }}">
